...
For demonstration purpose, we use the `Zhengmix4eq` dataset in the package with top 100 highly variable genes.
```{r, message=FALSE}
stats <- modelGeneVar(Zhengmix4eq_sce)
chosen <- getTopHVGs(stats, n = 100)
Zhengmix4eq_sce_sub <- Zhengmix4eq_sce[chosen, ]
```

Before simulation, we store two dimension reduction results in the `SingleCellExperiment` object.
```{r, message=FALSE, warning = FALSE}
Zhengmix4eq_sce_sub <- runPCA(Zhengmix4eq_sce_sub)
Zhengmix4eq_sce_sub <- runUMAP(Zhengmix4eq_sce_sub)
...
```
```{r, fig.width=12,fig.show='hide', message=FALSE, warning=FALSE}
...
p_vine_copula <- cowplot::plot_grid(plotlist = p_vine_list , nrow = 1, align = "hv", axis = "lr")
```

For the four plots on the top, they are generated using the `corr_list` returned when we use Gaussian copula. For the four plots on the bottom, they are generated using the `corr_list` returned when we use vine copula. The bottom plots give more direct visualization about which genes are connected in the vine structure, in the other words, which genes are closer to each other.
```{r, fig.width=12, message=FALSE}
p_vine <- cowplot::plot_grid(p1, p2, p3, p4, nrow = 1, align = "hv")

cowplot::plot_grid(corr_p, p_vine, nrow = 2, align = "hv", axis = "lr" )
```
