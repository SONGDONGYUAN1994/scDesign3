---
title: "Evaluate clustering qualities using scDesign3"
author: 
  - name: Dongyuan Song
    affiliation:
...
## Introduction
In this tutorial, we will show how to use scDesign3 to evaluate the clustering quailty for different cell-type assignments.
...
For demonstration purpose, we use the `Zhengmix4eq` dataset in the package with top 100 highly varaible genes and the corresponding k-means clustering result with k = $2 ,\cdots, 10$.
```{r, message=FALSE}
...
## Visualization
After the simulations, we can check the AIC provided by our package and the calculated ARI to evaluate clustering qualities.
```{r, message=FALSE}
aic_list <- lapply(scDesign3_result, function(x){return(x$model_aic)})
kmeans_ari <- sapply(kmeans_res, function(x){ARI(x$cluster, x$trueclass)})
aic_df <- data.frame(matrix(unlist(aic_list), nrow = length(aic_list), byrow = TRUE))
colnames(aic_df) <- names(aic_list[[1]])
rownames(aic_df) <- paste0("k = ", 2:10)
metric <- tibble(ari = kmeans_ari, aic = aic_df$aic.marginal, Method = paste0("k = ", 2:10))
p_cluster_metric <- metric %>% ggplot(aes(x =ari, y = aic,label = Method)) + geom_point() + theme_bw() + theme(aspect.ratio = 1,
    panel.grid.minor = element_blank(),
    panel.grid.major = element_blank()) + ggpubr::stat_cor(method = "spearman", cor.coef.name = "rho", label.x.npc = "left", label.y.npc = 0.5) + ylab("scDesign3 AIC") + xlab("ARI")
p_cluster_metric
```
