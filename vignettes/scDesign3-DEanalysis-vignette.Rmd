---
title: "DE Analysis with scDesign3"
author: 
  - name: Dongyuan Song
    affiliation:
...
theme_set(theme_bw())
```
In this tutorial, we will demonstrate how to use scDesign3 to generate negative control and benchmark methods for identifying differentially expressed(DE) genes between discrete cell types or continuous trajectory from scRNA-seq data, and identifying spatially variable genes (SVG) from spatial transcriptomics data.
...
dec.pbmc <- modelGeneVar(Zhengmix4eq_sce)
...
```{r, message=FALSE, warning=FALSE, results='hide'}
pbmc <- CreateSeuratObject(counts = example_newcount, project = "pbmc", min.cells = 0, min.features = 0)
...
qvals <- matrix(0, nrow = dim(pbmc)[1], ncol = length(test))
for (x in 1:length(test)) {
  markers <- FindMarkers(pbmc, ident.1 = "b.cells", ident.2 = "regulatory.t", test.use = test[x],
                         logfc.threshold = 0, min.pct = 0, min.cells.feature = 1, min.cells.group = 1)
  qvals[,x] <- p.adjust(markers[rownames(pbmc),"p_val"], method = "BH", length(rownames(pbmc)))
}
colnames(qvals) <- test
rownames(qvals) <- rownames(pbmc)
```
...
Now, we use the simulated data to benchmark the performance of two DEG identification methods: [Monocle3](https://www.nature.com/articles/nmeth.4636), [tradeSeq](https://www.nature.com/articles/s41467-020-14766-3), [NBAMseq](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-020-3506-x), and [PseudotimeDE](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-021-02341-y). The p-values from the two methods after Benjamini-Hochberg(BH) corretion will be stored in `qvals` in the following code.
```{r, message=FALSE, warning=FALSE, results='hide'}
...
####  Monocle3 
We follow the [tutorial](https://cole-trapnell-lab.github.io/monocle3/docs/differential/) from `Monocle 3` to conduct the DE test and obtain the p-values after BH correction.
```{r, message=FALSE, warning=FALSE, results='hide'}
...
#### NBAMseq
We follow the [tutorial](https://statomics.github.io/tradeSeq/articles/tradeSeq.html) from `NBAMseq` to conduct the DE test and obtain the p-values after BH correction.
```{r, message=FALSE, warning=FALSE, results='hide'}
...
gsd = NBAMSeq(gsd) 
res1 = results(gsd, name = "pseudotime")
head(res1)
...
```
Since `tradeSeq`'s result contains many NA, we convert the NA to 1 first.
```{r, message=FALSE, warning=FALSE, results='hide'}
qvals[is.na(qvals)] <- 1
print(head(qvals))
```
...
```
Here, we examine the `mean_mat`, which is one of the outputs from the previous function `extract_para()`. For each gene, we calculate the difference in the between the maximum mean parameter and minimum mean parameter across all cells. We select genes which the gene's mean difference across cells are in the top 10 largest differences. We regard these genes as spatially variable genes (SVGs). Then, we manually set the mean parameters of the rest genes to be the same across all cells. We regard all genes with the same mean parameter across cells as non-svg genes.
```{r,message=FALSE, warning=FALSE, results='hide'}
...
Here is another way to select SVG based on Moran's I.
```{r}
# num_de <- 50
...
long$x = rep(loc[,1],5)
long$y = rep(loc[,2],5)
as_tibble(long) %>% ggplot() +geom_point(aes(x=x,y=y,color=V1), size = 0.1)+facet_grid(~gene)+ scale_colour_gradientn(colors = viridis_pal(option = "magma")(10), limits=c(0, 1)) + coord_fixed(ratio = 1) +  theme_gray()
```
...
long$x = rep(loc[,1],5)
long$y = rep(loc[,2],5)
as_tibble(long) %>% ggplot() +geom_point(aes(x=x,y=y,color=V1), size = 0.5)+facet_grid(~gene)+ scale_colour_gradientn(colors = viridis_pal(option = "magma")(10), limits=c(0, 1)) + coord_fixed(ratio = 1) +  theme_gray()
```



### SVG identification
Now, we use the simulated data to benchmark the performance of two SVG identification methods: [spatialDE](https://www.nature.com/articles/nmeth.4636), [SPARK](https://www.nature.com/articles/s41592-019-0701-7), and [SPARK-X](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-021-02404-0). The p-values from the two methods after Benjamini-Hochberg(BH) corretion will be stored in `qvals` in the following code.
```{r, message=FALSE, warning=FALSE, results='hide'}
