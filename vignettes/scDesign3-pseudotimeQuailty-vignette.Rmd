---
title: "Evaluate psedotime qualities using scDesign3"
author: 
  - name: Dongyuan Song
    affiliation:
...
## Introduction
In this tutorial, we will first use the package `dyngen` to generate a dataset with ground truth \"pseudotime\". Then, we will perturb the ground truth pseudotime to worsen its quality and use scDesign3's AIC and BIC to examine different pseudotime's quality. 

## Generation of reference dataset & Simulation
We will use `dyngen` to generate a dataset with ground truth "pseudotime".
...
example_sce <- as_sce(out$model)
colData(example_sce)$pseudotime <- out$dataset$progressions$percentage 
```
Secondly, we perturb the generated time by percentage, ranging from 0% to 100%.
```{r, message=FALSE}
...
## Visualization
After the simulation, we can plot the BIC values vs $r^2$.
```{r, message=FALSE}
